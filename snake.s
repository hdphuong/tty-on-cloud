...
#
#
# This program was written by MAE-VUONG (z5286353)
# on 10/07/2021 
#
...
main__body:
	jal	init_snake		# call (void) init_snake
	jal	update_apple		# call (void) update_apple
					
main__loop:				
	jal	print_grid		# call (void) print_grid
	jal	input_direction		# call (int) input_direction
	
	move	$a0, $v0		# update_snake(direction)
	jal	update_snake
	move	$s0, $v0		
					
	beq	$s0, $zero, main__end# while (update_snake(direction)) 
	j	main__loop	
...
main__end:
	...
	la	$a0, main__game_over	# printf("Game over!...");
	li	$v0, 4
	syscall
...
update_apple:
	...
	#
	# Frame:    $ra, $s0, $s1, $s2 
	# Uses:     $a0, $v0, $t0, $t1, $t2, $t3, $t4, $t5,  $s0, $s1, $s2
	# Clobbers: uses - frame
	#
	# Locals:
	#   -  `int apple_row` in $s0
...
update_apple__loop:
	...
	li	$t0, N_COLS
	mul	$t0, $t0, 1		# calculate &grid[apple_row][apple_col]
	mul	$t1, $t0, $s0
	mul	$t2, $s1, 1
	add	$t3, $t1, $t2

	lb	$t4, grid($t3)
	bne	$t4, EMPTY, update_apple__loop	# while (grid[apple_row][apple_col] != EMPTY)

	li	$t5, APPLE		# grid[apple_row][apple_col] = APPLE
	sb	$t5, grid($t3)
...
update_snake:
	...
	#   - grid[new_head_row][new_head_col] in $s7
	#
	# Structure:
	#   update_snake
	#   -> [prologue]
...
update_snake__body:
	...
	move	$a0, $s0		# int d_row = get_d_row(direction)
	jal	get_d_row
	move	$s1, $v0

	move	$a0, $s0		# int d_col =  get_d_col(direction)
	jal	get_d_col	
	move	$s2, $v0	

	lb	$s3, snake_body_row	# head_row = snake_body_row[0]
	lb	$s4, snake_body_col	# head_col = snake_body_col[0]
	
	li	$t0, N_COLS		# calculate &grid[head_row][head_col]
	mul	$t0, $t0, 1	
	mul	$t0, $t0, $s3
	mul	$t1, $s4, 1	
	add	$t2, $t1, $t0
	...
	li	$t4, SNAKE_BODY 
	sb	$t4, grid($t2)		# grid[head_row][head_col] = snake_body
	
	add	$s5, $s3, $s1		# int new_head_row = head_row + d_row
	add	$s6, $s4, $s2		# int new_head_col = head_col + d_col
	
	blt	$s5, $zero, update_snake_return_false	# if (new_head_row < 0 ) 
	bge	$s5, N_ROWS, update_snake_return_false	# if (new_head_row >= N_ROWS)
	blt	$s6, $zero, update_snake_return_false		# if (new_head_col < 0) 
	bge	$s6, N_COLS, update_snake_return_false	# if (new_head_col >= N_COLS) 
	
	li	$t0, N_COLS		# calculate &grid[new_head_row][new_head_col]
	mul	$t0, $t0, 1
	mul	$t0, $t0, $s5
	mul	$t1, $s6, 1
	add	$t5, $t1, $t0	
	lb	$s7, grid($t5)		# $s7 = grid[new_head_row][new_head_col]
	
	lw	$t6, snake_body_len	# snake_tail = snake_body_len - 1
	sub	$t6, $t6, 1	
	sw	$t6, snake_tail

	move	$a0, $s5		# $a0 = new_head_row
	move	$a1, $s6		# $a1 = new_head_col
	jal	move_snake_in_grid	# if (!move_snake_in_grid)
	move	$t0, $v0		# return false
	beq	$t0, 0, update_snake_return_false
	
	move	$a0, $s5		# move_snake_in_array(new_head_row, new_head_col)
	move	$a1, $s6
	jal	move_snake_in_array

	beq	$s7, APPLE, apple_true	# if grid[new_head_row][new_head_col] == APPLE
	j	update_snake_return_true	
...
move_snake_in_grid:
	...
	# Uses:     $a0, $a1, $t0 -> $t9, $s0 -> $s3
	# Clobbers: uses - frames
	#
	# Locals:
	#   - `int tail` in $t4
	#   - `int tail_row` in $t5
	...
	#   -> [prologue]
	#   -> body
	#	-> move_snake_in_grid__true
	#	-> move_snake_in_grid__false
	#   -> [epilogue]
	#	-> move_snake_return_false
	#	-> move_snake_return_true
...
move_snake_in_grid__body:
	...
	lw	$t0, snake_growth
	bgt	$t0, $zero, move_snake_in_grid__true	# if (snake_growth > 0) goto true
	j	move_snake_in_grid__false
...
move_snake_in_grid__true:
	...
	lw	$t2, snake_body_len	# snake_body_len++
	addi	$t2, $t2, 1
	sw	$t2, snake_body_len	

	lw	$t3, snake_growth
	addi	$t3, $t3, -1		# snake_growth--
	sw	$t3, snake_growth	
	j	move_snake_in_grid__continue	

move_snake_in_grid__false:
	lw	$t4, snake_tail		# int tail = snake_tail;
	
	lb	$t5, snake_body_row($t4)	# int tail_row = snake_body_row[tail]
	lb	$t6, snake_body_col($t4)	# int tail_col = snake_body_col[tail]

	li	$t7, N_COLS
	mul	$t7, $t7, 1		# calculate &grid[tail_row][tail_col]
	mul	$t7, $t5, $t7
	mul	$t8, $t6, 1
	add	$t9, $t8, $t7
	
	li	$s2, EMPTY		# grid[tail_row][tail_col] = EMPTY
	sb	$s2, grid($t9)		
	j	move_snake_in_grid__continue


move_snake_in_grid__continue:
	li	$t7, N_COLS
	mul	$t7, $t7, 1		# calculate grid[new_head_row][new_head_col]
	mul	$t7, $a0, $t7
	mul	$t8, $a1, 1	
	add	$t9, $t7, $t8
	...
	lb	$s2, grid($t9)
	beq	$s2, SNAKE_BODY, move_snake_return_false

	li	$s3, SNAKE_HEAD		# grid[new_head_row][new_head_col] = SNAKE_HEAD
	sb	$s3, grid($t9)

	j	move_snake_return_true	# return true;
...
move_snake_in_array__loop:
	blt	$s0, 1, move_snake_in_array__end 
	
	addi	$t0, $s0, -1
	lb	$a0, snake_body_row($t0)	# $a0 = snake_body_row[i-1]
	lb	$a1, snake_body_col($t0)	# $a1 = snake_body_col[i-1]
	move	$a2, $s0			# $a2 = i
	jal	set_snake_array			# set_snake_array($a0, $a1, $a2);
